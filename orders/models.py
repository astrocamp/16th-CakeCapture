...
class Order(models.Model):
    ...
    def add_bonus_points(self):
        if self.buyer:
            bonus_points = round(self.total * 0.05)
            bonus, created = Bonus.objects.get_or_create(user=self.buyer)
...
class OrderMethod(models.Model):
    ...
    payment_method = models.CharField(
        max_length=20, choices=PAYMENT_CHOICES, default="信用卡"
    )
